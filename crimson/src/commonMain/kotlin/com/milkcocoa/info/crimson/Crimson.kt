...
class Crimson<SND: CrimsonData, RCV: CrimsonData>(
    ...
    override suspend fun execute(command: CrimsonCommand) {
        when(command){
            is CrimsonCommand.Ping -> webSocketSession?.send(Frame.Ping(command.frame))
            is CrimsonCommand.Pong -> webSocketSession?.send(Frame.Pong(command.frame))
            is CrimsonCommand.StartHealthCheck -> {
                this.webSocketSession ?: return
                this.incomingFrameFlow ?: return
                launchHealthCheck(this@Crimson.incomingFrameFlow!!)
            }
            is CrimsonCommand.Connect -> {
                mutex.withLock {
                    runCatching {
                        ...
                        this@Crimson.webSocketSession = it

                        this@Crimson.incomingFrameFlow = it.incoming
                            .consumeAsFlow()
                            .shareIn(
                        ...
                        handleIncomingFrame(this@Crimson.incomingFrameFlow!!)
                        respondToPingFrame(this@Crimson.incomingFrameFlow!!)
                        closeMonitor(this@Crimson.webSocketSession!!)
    ...
    private fun launchHealthCheck(sessionFlow: SharedFlow<Frame>) {
        var lastHealthCheckTime = Clock.System.now()

        sessionFlow.filterIsInstance<Frame.Pong>()
